...
@section('title', 'Gestión de Estados de Residentes')

@section('content_header')
    <h1>Gestión de Estados de Residentes</h1>
@stop

@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Listado de Estados de Residentes</h3>
            <div class="card-tools">
                <div class="d-flex justify-content-end">
                    <button class="btn btn-primary mr-2" data-toggle="modal" data-target="#nuevoEstadoModal">Nuevo</button>
                    <form id="reporteForm" method="GET" action="{{ route('estados_reservacion.reporte') }}" target="_blank">
    ...
    @if($hasPermission)
        <div class="table-container">
            <table id="estados-usuario-table" class="table table-striped table-bordered shadow-lg mt-4" style="width: 100%">
                <thead class="bg-primary text-white">
                    <tr>
                        <th>ID ESTADO DE RESERVACION</th>
                        <th>DESCRIPCIÓN</th>
                        <th>ACCIONES</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($EstadoReservacion as $estado)
                        <tr>
                            <td>{{ $estado["ID_ESTADO_RESERVA"] }}</td>
                            <td>{{ $estado["DESCRIPCION"] }}</td>
                            <td>
                                <button type="button" class="btn btn-info" data-toggle="modal" data-target="#editarEstadoModal{{ $estado['ID_ESTADO_RESERVA'] }}">Editar</button>
                                <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#eliminarEstadoModal{{ $estado['ID_ESTADO_RESERVA'] }}">Eliminar</button>
                            </td>
                        </tr>

                        <!-- Modal de editar estado -->
                        <div class="modal fade" id="editarEstadoModal{{ $estado['ID_ESTADO_RESERVA'] }}" tabindex="-1" role="dialog" aria-labelledby="editarEstadoModalLabel{{ $estado['ID_ESTADO_RESERVA'] }}" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="editarEstadoModalLabel{{ $estado['ID_ESTADO_RESERVA'] }}">Editar Estado</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body">
                                        <form class="editar-estado-form" data-id="{{ $estado['ID_ESTADO_RESERVA'] }}">
                                            @csrf
                        ...
                        <div class="modal fade" id="eliminarEstadoModal{{ $estado['ID_ESTADO_RESERVA'] }}" tabindex="-1" role="dialog" aria-labelledby="eliminarEstadoModalLabel{{ $estado['ID_ESTADO_RESERVA'] }}" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="eliminarEstadoModalLabel{{ $estado['ID_ESTADO_RESERVA'] }}">Eliminar Estado</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body">
                                        ...
                                        <form class="eliminar-estado-form" data-id="{{ $estado['ID_ESTADO_RESERVA'] }}">
                                            @csrf
        ...
        @else
            <div class="alert alert-danger">
                No tienes permisos para ver los Estados de residentes.
            </div>
        @endif
    </div>
...
function permitirSoloNumerosPositivos() {
    ...
    permitirSoloNumerosPositivos();
     ...
     $('#nuevo-estado-form').on('submit', function(event) {
            ...
            $.ajax({
                url: '{{ route("estados_reservacion.guardar") }}',
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
 ...
 $('.editar-estado-form').on('submit', function(event) {
            ...
            $.ajax({
                url: '{{ route("estados_reservacion.actualizar", ":id") }}'.replace(':id', id),
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        $('.eliminar-estado-form').on('submit', function(event) {
            ...
            $.ajax({
                url: '{{ route("estados_reservacion.eliminar", ":id") }}'.replace(':id', id),
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
    ...
    window.open('{{ route("estados_reservacion.reporte") }}', '_blank');
});
});
