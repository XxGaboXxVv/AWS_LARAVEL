        ...
        <div class="card-header">
            ...
            <div class="card-tools">
                <div class="d-flex justify-content-end">
                    ...
                    <form id="reporteForm" method="GET" action="{{ route('estados_persona.reporte') }}" target="_blank">
            ...
            <table id="estados-usuario-table" class="table table-striped table-bordered shadow-lg mt-4" style="width: 100%">
                ...
                <tbody>
                    @foreach($EstadoPersona as $estado)
                        <tr>
                            <td>{{ $estado["ID_ESTADO_PERSONA"] }}</td>
                            <td>{{ $estado["DESCRIPCION"] }}</td>
                            <td>
                                <button type="button" class="btn btn-info" data-toggle="modal" data-target="#editarEstadoModal{{ $estado['ID_ESTADO_PERSONA'] }}">Editar</button>
                                <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#eliminarEstadoModal{{ $estado['ID_ESTADO_PERSONA'] }}">Eliminar</button>
                            </td>
                        </tr>

                        <!-- Modal de editar estado -->
                        <div class="modal fade" id="editarEstadoModal{{ $estado['ID_ESTADO_PERSONA'] }}" tabindex="-1" role="dialog" aria-labelledby="editarEstadoModalLabel{{ $estado['ID_ESTADO_PERSONA'] }}" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="editarEstadoModalLabel{{ $estado['ID_ESTADO_PERSONA'] }}">Editar Estado</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body">
                                        <form class="editar-estado-form" data-id="{{ $estado['ID_ESTADO_PERSONA'] }}">
                                            @csrf
                        ...
                        <div class="modal fade" id="eliminarEstadoModal{{ $estado['ID_ESTADO_PERSONA'] }}" tabindex="-1" role="dialog" aria-labelledby="eliminarEstadoModalLabel{{ $estado['ID_ESTADO_PERSONA'] }}" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="eliminarEstadoModalLabel{{ $estado['ID_ESTADO_PERSONA'] }}">Eliminar Estado</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body">
                                        ...
                                        <form class="eliminar-estado-form" data-id="{{ $estado['ID_ESTADO_PERSONA'] }}">
                                            @csrf
...
function permitirSoloNumerosPositivos() {
    ...
    permitirSoloNumerosPositivos();
     ...
     $('#nuevo-estado-form').on('submit', function(event) {
            ...
            $.ajax({
                url: '{{ route("estados_persona.guardar") }}',
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
 ...
 $('.editar-estado-form').on('submit', function(event) {
            ...
            $.ajax({
                url: '{{ route("estados_persona.actualizar", ":id") }}'.replace(':id', id),
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        $('.eliminar-estado-form').on('submit', function(event) {
            ...
            $.ajax({
                url: '{{ route("estados_persona.eliminar", ":id") }}'.replace(':id', id),
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
    ...
    window.open('{{ route("estados_persona.reporte") }}', '_blank');
});
