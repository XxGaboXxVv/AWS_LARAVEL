 ...
 public function bitacora()
    {
        ...
        return view('BitacoraVisita', compact('bitacoraVisitas', 'personas', 'visitantes', 'visitantesRecurrentes','hasPermission'));
    }
    
    public function fetchBitacoraVisita(Request $request)
{
    ...
    if ($search) {
        $bitacoraVisitas = array_filter($bitacoraVisitas, function ($visita) use ($search) {
            return stripos($visita['PERSONA'], $search) !== false ||
                   stripos($visita['VISITANTE'], $search) !== false ||
                   stripos($visita['NUM_PLACA'], $search) !== false ||
                   stripos($visita['VISITANTE_RECURRENTE'], $search) !== false ||
                    stripos($visita['FECHA_HORA'], $search) !== false;

        });
    }

    // Paginación
    $totalData = count($bitacoraVisitas);
    $totalFiltered = count($bitacoraVisitas);
    $bitacoraVisitas = array_slice($bitacoraVisitas, $start, $length);

    return response()->json([
        "draw" => intval($request->input('draw')),
        "recordsTotal" => $totalData,
        "recordsFiltered" => $totalFiltered,
        "data" => $bitacoraVisitas
    ]);
}
